...
class eSSP(object):
    ...
    def __init__(self, com_port, ssp_address="0", nv11=False, debug=False):
        ...
        #Add variables
        self.channel_cash = {} #словарь 'порядковый номер':"номинал банкноты"
        self.credit_cash_list = [] #список поступивших банкнот
        self.running = False #Валидатор в действии
        ...
        for i, channel in enumerate(setup_req.ChannelData):
            self.channel_cash[i+1] = channel.value
        ...
        for events in self.poll.events:
            ...
            if events.event == Status.SSP_POLL_RESET:
                ...
                if events.data1 > 0:
                    ...
                    self.credit_cash_list.append(self.channel_cash[events.data1])
    ...
    def get_last_credit_cash(self):
        """Get the last credit cash and delete from the credit list"""
        credit_cash = 0
        if(len(self.credit_cash_list) >= 1):
            credit_cash = self.credit_cash_list[len(self.credit_cash_list) - 1]
            self.credit_cash_list.pop(len(self.credit_cash_list) - 1)
        return credit_cash
